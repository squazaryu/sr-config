...
нодами по пингу. На iOS она задана как политика Spotify по умолчанию. В ручной
группе выбора сервера предпочтительная iOS-подписка — `PREMIUM | ALL IN 1`;
`YOUR-DUREV.COM` остаётся доступной как запасной.
...
На iOS встроенные правила Instagram/Meta используют политику `PROXY` напрямую.
Instagram следует за конкретным сервером, выбранным на главном экране
Shadowrocket, и не проходит через вложенную группу `PREMIUM | ALL IN 1`.
