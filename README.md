...
По умолчанию она использует проверенный узел `🇫🇮 Финляндия`; `DIRECT`,
`🗺️ Выбор сервера` и `PROXY` остаются ручными запасными вариантами.
GitHub-источники Feather сохраняют подтверждённую маршрутизацию `DIRECT`, а
iApps Store закреплён через `DIRECT` до внешних списков.
