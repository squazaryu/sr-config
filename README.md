...
На iOS AI-сервисы используют отдельную группу `🤖 AI-сервисы v2` типа
`url-test`, точно перенесённую из экспорта iPhone от 2026-09-04 10:12:53.
В ней шесть финских policy в порядке: PROXY TG, ДАРВИН ВПН, SODA VPN,
HIT VPN, FASTCOM VPN, ALL VPN. `policy-select-name` задан как
`🇫🇮 ДАРВИН ВПН | ФИНЛЯНДИЯ`, параметры проверки — `interval=600`,
`tolerance=100`, `timeout=5`, `url=http://www.gstatic.com/generate_204`.
Обычный `PROXY`, общая финская auto-группа и расширение из подписки в AI-группу
не входят. FASTCON VPN остаётся в общей `🇫🇮 Финляндия (авто)`, но не в AI-группе.
Тип `url-test` может выбрать другой узел; успешная проверка gstatic не
подтверждает доступность авторизации или API нейросетей. Пользователь подтвердил
работу этого варианта на iPhone, длительная стабильность ещё не подтверждена.
macOS и самодостаточный failsafe сохраняют независимую финскую маршрутизацию.
