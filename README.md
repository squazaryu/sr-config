...
На iOS группа `📱 Instagram (один узел)` по умолчанию использует конкретный
узел `🇵🇱 ALL VPN | Польша`. Это предотвращает смену страны выхода внутри
одной Instagram-сессии; альтернативный узел и прежние авто-группы доступны
