...
На iOS `🌍 Общий прокси (ALL VPN FI)` и завершающее правило `FINAL` используют
`🇫🇮 ALL VPN | ФИНЛЯНДИЯ`. Так общие и нераспознанные запросы сохраняют тот же
выход, что и AI, независимо от сервера на главном экране. Это осознанное
отличие от macOS и прежнего iOS-пути через `PREMIUM | ALL IN 1`.
...
На iOS `🤖 AI-сервисы (ALL VPN FI)` типа `select` содержит единственную policy
`🇫🇮 ALL VPN | ФИНЛЯНДИЯ`, с ней же в `policy-select-name`. AI не использует
`PROXY`, auto-группу, URL-тест или автоматический fallback. Новые имена AI и
общей группы отделяют их от старого сохранённого выбора.
...
В F08 этот выход зависел от выбранного на главном экране сервера; теперь
AI, общий прокси и FINAL закреплены явно. Это изменение способа выбора узла,
и его работу после обновления нужно подтвердить на iPhone.
...
`sub.alvsub.cc`, `your-durev.com` через фиксированный финский общий прокси.
...
не профиль для установки; тест проверяет только согласованные отличия от него.
...
полностью переподключи VPN и перезапусти ChatGPT. В группах должны отображаться
`AI-сервисы (ALL VPN FI) → ALL VPN | ФИНЛЯНДИЯ` и такой же узел у общего прокси.
...
Фиксация policy исключает выбор другого кандидата этим конфигом, но не
гарантирует постоянный IP, его геолокацию у AI-провайдера или доступность сервера.
При отказе ALL VPN автоматического перехода AI на другой узел нет. Тесты и CI
проверяют файл, а не успешность запросов к ChatGPT на iPhone.
...
|---|---|---|
| `lists/ai-services.list` | 🤖 AI-сервисы (ALL VPN FI) (iOS) / 🤖 AI-сервисы (macOS) | OpenAI/ChatGPT, Claude, Gemini, Genspark, Manus |
...
| `lists/general-proxy.list` | 🌍 Общий прокси (ALL VPN FI) (iOS) / 🌍 Общий прокси (macOS) | Google/YouTube, Meta, Discord, GitHub, прочее |
