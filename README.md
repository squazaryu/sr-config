...
по категориям (AI-сервисы, Spotify, Погода, Telegram, Feather,
общий прокси, блокировка рекламы) можно
...
На iOS каталоги и файловые хосты Feather выделены в группу `🪶 Feather`.
По умолчанию она использует встроенную policy `PROXY`, то есть выбранный на
главном экране рабочий сервер; `🇫🇮 Финляндия (авто)`, `DIRECT` и
`🗺️ Выбор сервера` остаются ручными вариантами. GitHub-источники Feather
сохраняют подтверждённую маршрутизацию `DIRECT`, а iApps Store закреплён через
`DIRECT` до внешних списков.
