...
### Как обновлять без дублирования

Каждый URL добавляй в Shadowrocket один раз через `Конфигурация → + → URL`.
После этого используй действие `Обновить конфигурацию` у уже добавленного
файла. Не добавляй тот же URL повторно через `+`: это создаёт новую локальную
копию, а не обновляет существующую.

`Использовать конфигурацию` / `Компилировать конфигурацию` обновляет внешние
`RULE-SET` и применяет их, но не заменяет сам удалённый файл. Не запускай
ручное обновление одновременно с автоматическим фоновым обновлением или
компиляцией.

Если в списке уже появились повторяющиеся правила, сохрани нужные локальные
настройки, удали повреждённую копию конфигурации и импортируй соответствующий
URL заново один раз. Конфиги iOS и macOS импортируются раздельно.

Если после чистого импорта дубли снова появляются, проверь версию Shadowrocket
в App Store: это уже проблема локальной синхронизации или компиляции клиента,
которую статический конфиг на GitHub исправить не может. Серверный файл должен
содержать одну копию каждой секции; это дополнительно проверяется в CI.
...
В iOS-конфиге перед всеми внешними списками также находится локальное
bootstrap-правило `DOMAIN,raw.githubusercontent.com,PROXY`. Оно нужно для
первичной загрузки `RULE-SET`: маршрут к GitHub не должен зависеть от уже
загруженного внешнего списка `general-proxy.list`. Группа `🌍 Общий прокси`
по умолчанию использует встроенную политику `PROXY`, то есть текущий узел,
выбранный на главном экране Shadowrocket. Это не позволяет устаревшей или
несовместимой ручной группе серверов блокировать загрузку списков. В iOS
селектор `🗺️ Выбор сервера` также не содержит фиксированных имён старых
подписок: он предлагает текущий узел, рабочую финскую группу или `DIRECT`.
