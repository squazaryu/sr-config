...
Локальный установщик Feather использует `*.backloop.dev`, который должен
возвращать настоящий loopback-адрес и обходить прокси. Сам `backloop.dev`
для загрузки SSL-пакета и `api.palera.in` для Semi Local manifest остаются через
`PROXY`. Эти правила включены в iOS-конфиг и failsafe; macOS-конфиг не меняется.
